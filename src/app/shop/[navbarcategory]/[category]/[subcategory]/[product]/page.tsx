...
export default function ProductPage({
  ...
  const fetchCategoryAndSubcategory = async () => {
    if (product?.categoryId) {
      try {
        const categoryRes = await fetch(`/api/categories/${product.categoryId}`);
        if (categoryRes.ok) {
    ...
    if (product?.subcategoryId) {
      try {
        const subcategoryRes = await fetch(`/api/subcategories/${product.subcategoryId}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Failed to create catalog request');
      }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
      ...
      <div className="space-y-4 mt-12">
        <div className="bg-white/90 backdrop-blur-lg rounded-xl p-6 shadow-[0_8px_32px_rgba(31,_38,_135,_0.1)] border border-white/20 relative overflow-hidden">
          ...
          <h3 className="text-xl font-semibold mb-4 text-gray-900 relative z-10">Request Catalog</h3>
          ...
          <div className="fixed inset-x-0 top-[100px] bottom-0 z-50 flex items-start justify-center overflow-hidden p-4">
            <div className="bg-white/95 backdrop-blur-xl rounded-xl max-w-md w-full shadow-[0_8px_32px_rgba(31,_38,_135,_0.2)] animate-modalSlideIn border border-white/20">
              <div className="p-6 border-b flex items-center justify-between">
                <h2 className="text-xl font-semibold text-gray-900">Request Catalog</h2>
                <button
      ...
      <div className="text-sm text-gray-500 mb-4">
        <span>Navbar Category: {typeof product.navbarCategoryId === 'string' ? product.navbarCategoryId : product.navbarCategoryId?.name}</span>
        {categoryName && <span> / Category: {categoryName}</span>}
        {subcategoryName && <span> / Subcategory: {subcategoryName}</span>}
      </div>
    </div>
  );
}